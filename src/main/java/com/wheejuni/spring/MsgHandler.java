	...
	public static JSONObject readMessage(String message) throws Exception {
		...
		if (message.matches("인문관식당")) {
			...
			if (hour < 11) {
				...
				JSONObject lunch = CreateJSON
						.returnJSON(rawmenu[1].toString() + rawmenu[2].toString() + rawmenu[3].toString());
		...
		if (message.matches("업데이트 일정 안내")) {
			...
			JSONObject update = CreateJSON.returnJSON(
					"외대학식알리미는 2학기 개강 일정에 맞춰 완전히 업데이트될 예정입니다. \n \n새로운 버전에서는 글로벌캠퍼스 지원을 포함한 다양한 기능이 추가될 예정입니다. 감사합니다!");
